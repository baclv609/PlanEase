...
const onFinish = async (values) => {
    try {
        ...
        if (res.status === 200) {
            ...
            localStorage.setItem('token_expires_in', res.data.expiresIn);
            router.push('/');
        }
    } catch (error) {
...
<template>
    <div class="flex justify-center items-center h-[100vh]">
        <div class="flex w-full justify-items-center max-w-sm mx-auto overflow-hidden bg-white rounded-lg shadow-lg lg:max-w-4xl">
            ...
            <a-form layout="vertical" :model="formState" :rules="rule" name="basic" @finish="onFinish"
                ...
                <div class="flex items-center justify-center flex-col gap-4 mt-4">
                    <router-link to="#" class="no-underline text-xs text-blue-500 hover:underline">Quên mật
                        khẩu?</router-link>

                    <router-link to="/register" class="no-underline text-xs text-gray-500 uppercase hover:underline">HOẶC
                        ĐĂNG KÝ
                    </router-link>
